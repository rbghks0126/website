...
![CAVI algorithm](vi_review_cavi.jpg)
...
* We apply CAVI to a simple mixture of Gaussians, with K mixture components and n real-valied data points $x_{1:n}$. The latent variables are K real-valued mean parameters $\textbf{\mu}=\mu_{1:k}$ and n latent-class assignments $\textbf{c}=c_{1:n}$, where $c_i$ is an indicator (one-hot) K-vector.
* Derivations of the ELBO, and variational updates for the cluster assignment $c_i$ and k-th mixture component $\mu_k$ is shown [here](vi_review_cavi_derivation.pdf).